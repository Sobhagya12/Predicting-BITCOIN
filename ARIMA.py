...
output_scrap = open('datatimed_3.csv','w')
...
datatimed = pd.read_csv('datatimed_3.csv', index_col = 'Timestamp', parse_dates = True)
...
#df_test=pd.DataFrame(columns = ['Orignal'],['Predicted'])

Index=test_arima.index.tolist()
df_test = pd.DataFrame(list(zip(Index,originals, predictions,error_list)), 
               columns =['Index','Orignal', 'Predicted','Error %'])


for i in range(len(df_test)):
    df_test['Error %'][i]=float(df_test['Error %'][i][0])
    df_test['Index'][i]=df_test['Index'][i].split(" ")

df_test.to_csv('file3.csv', header=False, index=False) 
